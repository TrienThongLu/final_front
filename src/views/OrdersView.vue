    ...
    <div class="order-body">
      ...
      <div class="order-body-item">
        ...
        <div class="order-body-item_body">
          ...
          >
            <div
              class="order-body-item_body_item_header"
              :style="
                order.type == 'Online'
                  ? 'background-color: greenyellow'
                  : 'background-color: aquamarine'
    ...
    async GetOrders() {
      try {
        const res = await this.$mainAxios.get(
          `Order/GetOrders/${this.store.id}`
        );
        if (res.status == 200) {
          console.log(res);
          for (let orderList of res.data.ordersData) {
            switch (orderList.status) {
              case 0:
                this.ordersPendingPay = orderList.order;
                break;
              case 1:
                this.ordersProcessing = orderList.order;
                break;
              case 2:
                this.ordersDelivery = orderList.order;
                break;
              // case 3:
              //   this.ordersDelivery = orderList.order;
              //   break;
          ...
          this.ordersCompleted = res.data.completedOrders;
        }
      } catch (error) {
...
.order-body-item_body_item_body > p:first-child {
  white-space: nowrap;
