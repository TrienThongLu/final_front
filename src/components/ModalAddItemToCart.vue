      ...
      <div class="Modal-container">
        ...
        <div class="Modal-body">
          ...
          <div class="Modal-item-size">
            ...
            <div class="Modal-item-size_body">
              <div
                ...
                v-for="size in newItem.groupSizes"
                :key="size.name"
              >
                <input
                  type="radio"
          ...
          <div
            ...
            v-if="newItem.toppings.length != 0 && newItem.toppings"
          >
            ...
            <div
              ...
              v-for="topping in newItem.toppings"
              :key="topping"
            >
        ...
        <div class="Modal-footer">
          <button @click="addItemToCart" :disabled="quantityMax == 0">
            {{ toLocaleNumber(totalPrice) }}₫ - Add to Cart
          </button>
        </div>
      </div>
    </transition>
...
export default {
  ...
  watch: {
    ModalAddItemActive(value) {
      ...
      if (value) {
        ...
        this.newItem.image = this.Item.image;
        this.newItem.price = this.Item.price;
        this.newItem.typeId = this.Item.typeId;
    ...
    //   };
    //   this.totalPrice = 0;
    //   this.newItem.id = newValue.id;
    //   this.newItem.name = newValue.name;
    //   this.newItem.image = newValue.image;
    //   this.newItem.price = newValue.price;
    //   this.newItem.typeId = newValue.typeId;
    //   this.newItem.groupSizes = newValue.groupSizes;
    //   this.addItem.itemPrice = newValue.price;
    //   this.newItem.toppings = newValue.toppings;
    // },
    addItem: {
      handler(newValue) {
        this.totalPrice =
  ...
  computed: {
    // totalPrice() {
    //   // if (this.addItem.itemPrice) {
    //   return this.addItem.itemPrice * this.addItem.quantity;
    //   // }
    //   // return 0;
    // },
    // quantityMax() {
  ...
  methods: {
    ...
    reset() {
      ...
      this.totalPrice = this.initialState().totalPrice;
      this.Error = this.initialState().Error;
      this.newItem.toppings.forEach((val) => {
        val.quantity = 0;
      });
    },
    quantityMaxFunc() {
    ...
    plusTotalPrice(value) {
      this.addItem.itemPrice += value;
    },
    minusTotalPrice(value) {
      this.addItem.itemPrice -= value;
    },
    addItemToCart() {
      let toppings = this.newItem.toppings.filter((t) => t.quantity != 0);
      let itemObject = {
        id: this.newItem.id,
        name: this.newItem.name,
        image: this.newItem.image,
        typeId: this.newItem.typeId,
        size: this.addItem.size,
        basePrice: this.newItem.price,
        itemPrice: this.newItem.price + this.addItem.size.price,
        itemTotalPrice: this.addItem.itemPrice + this.addItem.size.price,
        toppings: toppings,
      };
      let itemContainer = {
        item: itemObject,
      ...
      if (!cart || cart.length == 0) {
        var newCart = [];
        newCart.push(itemContainer);
        localStorage.setItem("Cart", JSON.stringify(newCart));
        this.$mitt.emit("itemAdded", true);
      } else {
        cart.push(itemContainer);
        const res = cart.reduce((acc, e) => {
